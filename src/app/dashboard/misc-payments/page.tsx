...
export default function MiscPaymentsPage() {
  ...
  return (
    <PageShell title="کارمزد" description="مدیریت پرداخت‌های متفرقه"
    ...
    >
      <div className="rounded-md border bg-card p-4 mb-4">
        <div className="flex items-center justify-between">
          <span className="font-medium">مجموع پرداخت‌ها:</span>
          <span className="text-xl font-bold">{toPersianCurrency(totalAmount)}</span>
        </div>
      </div>
      <DataTable columns={columns} data={data?.results} totalCount={data?.count} page={page} onPageChange={setPage} searchable searchPlaceholder="جستجو..." onSearch={(term) => { setSearch(term); setPage(1); }} onEdit={openEdit} onDelete={handleDelete} emptyMessage="پرداختی ثبت نشده است" />
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader><DialogTitle>{selectedItem ? "ویرایش" : "پرداخت جدید"}</DialogTitle></DialogHeader>
