...
const productType = ProductType.SABOS_NARM;
const productLabel = "سبوس نرم";

export default function PurchasesSabosNarmPage() {
  const [page, setPage] = useState(1);
  const [search, setSearch] = useState("");
  const { data, isLoading } = usePurchases(productType, { page, search, page_size: 10 });
  const createMutation = useCreateMutation(API_ENDPOINTS.PURCHASES);
  const updateMutation = useUpdateMutation(API_ENDPOINTS.PURCHASES);
  const deleteMutation = useDeleteMutation(API_ENDPOINTS.PURCHASES);
  ...
  const openEdit = (item: Purchase) => {
    ...
    setFormData({
      ...
      purchase_date: item.transaction_date?.split("T")[0] || "", description: item.description || "",
    });
    setDialogOpen(true);
  };
  const handleDelete = (item: Purchase) => { setSelectedItem(item); setDeleteDialogOpen(true); };
  ...
  const handleSubmit = () => {
    const payload: Record<string, unknown> = {
      ...
      transaction_date: formData.purchase_date, description: formData.description || undefined,
    };
  ...
  return (
    <PageShell title={`خرید ${productLabel}`} description={`مدیریت خرید ${productLabel}`}
      actions={<Button onClick={openCreate}><Plus className="h-4 w-4 ml-1" />خرید جدید</Button>}
    >
      <DataTable columns={columns} data={data?.results} totalCount={data?.count} page={page} onPageChange={setPage} searchable searchPlaceholder="جستجوی خرید..." onSearch={(term) => { setSearch(term); setPage(1); }} onEdit={openEdit} onDelete={handleDelete} emptyMessage="خریدی ثبت نشده است" />
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader><DialogTitle>{selectedItem ? "ویرایش خرید" : "خرید جدید"}</DialogTitle></DialogHeader>
