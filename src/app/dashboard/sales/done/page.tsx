...
export default function SalesDonePage() {
  ...
  const openEdit = (item: Sale) => {
    ...
    setFormData({
      ...
      price_per_unit: String(item.unit_price),
      sale_date: item.transaction_date?.split("T")[0] || "",
  ...
  return (
    <PageShell
      title={`فروش ${productLabel}`}
      description={`مدیریت فروش ${productLabel}`}
      actions={
    ...
    >
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>{selectedItem ? "ویرایش فروش" : "فروش جدید"}</DialogTitle>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            <FormField label="مشتری">
              <CustomerSearch value={formData.customer} onChange={(c) => setFormData({ ...formData, customer: c })} />
            </FormField>
            <div className="grid grid-cols-2 gap-4">
              ...
              <FormField label="قیمت واحد (ریال)" required>
